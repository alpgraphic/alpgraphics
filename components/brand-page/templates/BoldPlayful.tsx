...
export default function BoldPlayful({ brandPage }: BoldPlayfulProps) {
    ...
    // Vibrant colors
    const primaryColor = colors.colors?.[0]?.hex || "#FF6B6B";
    const secondaryColor = colors.colors?.[1]?.hex || "#4ECDC4";
                ...
                >
                    {logos.dark ? (
                        <img
                            src={logos.dark}
                            alt={brandName}
                            className="max-w-[400px] max-h-[200px] object-contain mx-auto"
                        />
                    ) : (
                ...
                <div className="max-w-6xl mx-auto">
                    ...
                    >
                        ...
                        <div>
                            ...
                            <p className="text-lg text-black/60 mt-2">{textOverrides?.logoShowcaseSubtitle || "Our visual identity"}</p>
                        </div>
                    </motion.div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                        ...
                        >
                            {logos.light ? (
                                <img src={logos.light} alt="Primary" className="max-w-[70%] max-h-[70%] object-contain" />
                            ) : (
                                <span className="text-5xl font-black text-white">{brandName}</span>
                            )}
                        </motion.div>
                        ...
                        >
                            {logos.light ? (
                                <img src={logos.light} alt="On Primary" className="max-w-[80%] max-h-[80%] object-contain" />
                            ) : (
                                <span className="text-2xl font-black text-white">{brandName}</span>
                            )}
                        </motion.div>
                        ...
                        >
                            {logos.dark ? (
                                <img src={logos.dark} alt="On Secondary" className="max-w-[80%] max-h-[80%] object-contain" />
                            ) : (
                ...
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        >
                            ...
                            <p className="text-8xl font-black text-white mt-4">Aa</p>
                            <p className="text-xl font-bold text-white mt-4">{fonts.heading?.name || "Poppins"}</p>
                            <div className="mt-6 pt-6 border-t border-white/20">
                                <p className="text-3xl font-black text-white">GO BOLD OR GO HOME!</p>
                            </div>
                        </motion.div>
                        ...
                        >
                            ...
                            <p className="text-7xl font-bold mt-4">Aa</p>
                            <p className="text-xl font-medium mt-4">{fonts.body?.name || "Inter"}</p>
                            <div className="mt-6 pt-6 border-t border-black/20">
                                <p className="text-lg leading-relaxed">The quick brown fox jumps over the lazy dog.</p>
                            </div>
                        </motion.div>
                        ...
                        >
                            <p className="text-4xl md:text-5xl font-black text-white tracking-wider">
                                ABCDEFGHIJKLMNOPQRSTUVWXYZ
                            </p>
                            <p className="text-2xl md:text-3xl font-bold text-white/40 mt-4 tracking-wide">
                                0123456789 !@#$%
                            </p>
                        </motion.div>
                    </div>
                ...
                <div className="max-w-4xl mx-auto text-center">
                    ...
                    >
                        <h2 className="text-5xl md:text-7xl font-black mb-6">
                            <span style={{ color: primaryColor }}>{textOverrides?.footerTitle?.split(' ')[0] || "Let's"}</span>{' '}
                            <span style={{ color: secondaryColor }}>{textOverrides?.footerTitle?.split(' ').slice(1).join(' ') || "Talk!"}</span>
                        </h2>
                    ...
                    <div className="mt-24 pt-8 border-t border-white/10 flex justify-between items-center text-sm">
                        <span className="font-bold">© {new Date().getFullYear()} {brandName}</span>
