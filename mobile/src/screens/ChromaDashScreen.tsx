/**
 * Chroma Dash v5 — Apple Game Center Entegrasyonu
 *
...
const GC_LEADERBOARD_ID = 'com.alpgraphics.client.chromadash.highscores';
...
export default function ChromaDashScreen({ navigation }: Props) {
    ...
    // ── Push token alma (GC bildirimler için) ─────────────────────────────────
    ...
    const submitScoreToBackend = useCallback(async (score: number) => {
        ...
        try {
            ...
            await fetch(`${API_BASE_URL}/api/mobile/game/scores`, {
                method: 'POST',
                        ...
                        <View style={s.overlayBase} pointerEvents="box-none">
                            <ScrollView contentContainerStyle={s.scrollContent} showsVerticalScrollIndicator={false}>
                                ...
                                <TouchableOpacity
                                    ...
                                    activeOpacity={0.7}
                                >
                                    <View style={s.lbButtonRow}>
                                        <Text style={s.lbButtonIcon}>🏆</Text>
                                        <View style={{ flex: 1 }}>
                                            <Text style={[s.cardLabel, { color: '#00e5ff', marginBottom: 2 }]}>LİDERLİK TABLOSU</Text>
                                            <Text style={s.cardSub}>Game Center sıralamasını göster</Text>
                                        </View>
                                        <Text style={s.lbButtonArrow}>→</Text>
                                    </View>
                                </TouchableOpacity>

                                {/* Kişisel en iyi skorlar */}
                                {leaderboard.length > 0 && (
                                    <View style={s.card}>
                                        <Text style={s.cardLabel}>KİŞİSEL EN İYİLER</Text>
                                        {leaderboard.map((e, i) => {
                    ...
                    {gs.phase === 'dead' && (
                        <View style={s.overlayBase} pointerEvents="box-none">
                            <ScrollView contentContainerStyle={s.scrollContent} showsVerticalScrollIndicator={false}>
                                ...
                                {/* Game Center liderlik tablosu butonu */}
                                <TouchableOpacity
                                    style={[s.card, { borderColor: 'rgba(0,229,255,0.25)' }]}
                                    onPress={() => {
                                        if (gc.isReady) {
                                ...
                                >
                                    <View style={s.lbButtonRow}>
                                        ...
                                        <View style={{ flex: 1 }}>
                                            <Text style={[s.cardLabel, { color: '#00e5ff', marginBottom: 2 }]}>LİDERLİK TABLOSU</Text>
                                            <Text style={s.cardSub}>Sıralamana bak</Text>
                                        </View>
                                        <Text style={s.lbButtonArrow}>→</Text>
                                    </View>
                                </TouchableOpacity>
            ...
            <View style={[s.footer, { height: footerH, paddingBottom: insets.bottom }]}>
                {gs.phase === 'playing' ? (
                    ...
                    <View style={s.footerIdle}>
                        ...
                        <Text style={s.footerIdleTxt}>{gcPlayerName ? `🎮 ${gcPlayerName}` : 'alpgraphics'}</Text>
                    </View>
                )}
            </View>
        </View>
...
const s = StyleSheet.create({
    ...
    tagline: { fontSize: 10, color: 'rgba(255,255,255,0.28)', letterSpacing: 1, marginBottom: 12 },
    ...
    gcBadgeTxt: { fontSize: 11, fontWeight: '700', color: '#39ff6a', letterSpacing: 0.3 },
    ...
    cardSub: { fontSize: 11, color: 'rgba(255,255,255,0.38)', lineHeight: 17 },
    ...
    // ── Leaderboard buton ─────────────────────────────────────────────────────
    lbButtonRow: { flexDirection: 'row', alignItems: 'center', gap: 12 },
    lbButtonIcon: { fontSize: 28 },
    lbButtonArrow: { fontSize: 18, color: 'rgba(255,255,255,0.25)', fontWeight: '700' },

    // ── Leaderboard satırları (kişisel) ───────────────────────────────────────
    ...
    lbRankTxt: { fontSize: 11, color: 'rgba(255,255,255,0.4)', flex: 1, marginLeft: 6 },
